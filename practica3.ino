...
class IrrigationSystem {
  private:
    const char* shadow_update_topic = "$aws/things/proyecto_03/shadow/update";
    const char* shadow_update_accepted_topic = "$aws/things/proyecto_03/shadow/update/accepted";
    const int humidity_sensor_pin = 32;
    const int pump_pin = 5;
    const int humidity_threshold = 1300;
  ...
  public:
    ...
    void connectAWS() {
      ...
      mqtt_client.setCallback([this](char* topic, byte* payload, unsigned int length) { this->messageCallback(topic, payload, length); });
    ...
    void reportHumidity() {
      ...
      mqtt_client.publish(shadow_update_topic, output.c_str());
      Serial.print("Sending humidity level: ");
...
void setup() {
  ...
  digitalWrite(irrigation_system.pump_pin, LOW);
